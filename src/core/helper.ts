...
import BotsAppClass from './sidekick'
...
const resolve = function (messageInstance: proto.IWebMessageInfo, client: WASocket) {
    ...
    BotsApp.messageFrom = BotsApp.fromMe ? "Me" : users[messageInstance.key.participant] ? users[BotsApp.sender?.substring(0, BotsApp.sender.indexOf("@"))] : messageInstance.pushName;
    BotsApp.isDeleted = messageInstance.message?.protocolMessage?.type === 0 ? true : false;
    BotsApp.deletedMessageID = BotsApp.isDeleted ? messageInstance.message!.protocolMessage!.key.id : null;
    return BotsApp;
}
